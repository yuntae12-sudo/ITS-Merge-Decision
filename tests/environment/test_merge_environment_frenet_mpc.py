...
# ======================================================================
# Stage 3-H Section 1: real-scene regression for the Stage 3-G
# lane_geometry.py bug fix (project_point_to_polyline_signed's
# extrapolation-branch lateral_distance_m computation).
#
# This is a real-WOMD, end-to-end companion to
# tests/scenarios/test_lane_geometry.py's synthetic unit tests above.
# Before the Stage 3-G fix, MAN_0041's chain (618->623->608) NEVER
# advanced past active_transition_index=0 under frenet_mpc: projecting
# ego's real pose into the short (~20.5m) intermediate lane 623's own
# Frenet frame produced a corrupted lateral_distance_m of ~-44.6 m
# (true value ~-0.97 m), which fed a MERGE candidate whose lateral
# quintic swept ~44m in 3s -- violating curvature feasibility and
# spuriously colliding with unrelated real agents, permanently
# rejecting every MERGE candidate as PLANNER_INFEASIBLE/
# COLLISION_BLOCKED. If this exact bug were reintroduced, this test
# would fail: the chain would never advance and the episode would
# never reach a genuine `success` termination within the step budget
# below (confirmed in Stage 3-G's own diagnostic: post-fix, the chain
# advances at step 40 and the episode succeeds at step 59 under a
# fixed MERGE-every-step script).
# ======================================================================


def test_man_0041_chained_maneuver_reaches_success_under_repeated_merge_frenet_mpc():
    """Regression lock for the Stage 3-G lane_geometry.py fix: MAN_0041
    (the real chained maneuver whose chain never advanced pre-fix) must
    reach a genuine ``success`` termination within a bounded step
    budget under frenet_mpc when MERGE is selected every step (the same
    robustness stimulus used by scripts/audit_phase3_robustness.py).
    """
    ...
    max_steps = 120  # matches the Stage 3-G robustness audit's own cap
    chain_ever_advanced = False
    termination_reason = None

    for _ in range(max_steps):
        obs, _, terminated, truncated, info = env_instance.step(BehaviorAction.MERGE)
        assert np.all(np.isfinite(obs))
        if info["chain_advanced"]:
            chain_ever_advanced = True
        if terminated or truncated:
            termination_reason = info["termination_reason"]
            break

    assert chain_ever_advanced, (
        "MAN_0041's chain never advanced past active_transition_index=0 -- "
        "this is the exact symptom of the Stage 3-G lane_geometry.py bug "
        "(corrupted lateral_distance_m in project_point_to_polyline_signed's "
        "extrapolation branches)."
    )
    assert termination_reason == "success", (
        f"MAN_0041 was expected to reach a genuine success termination "
        f"under repeated MERGE (post Stage 3-G fix); got {termination_reason!r}."
    )


# ======================================================================
# Stage 3-H Section 3: explicit, countable downstream-intervention
# diagnostics (downstream_failure_count / planner_infeasible_count /
# collision_blocked_count / intervention_rate).
#
# These fields make the existing bounded-brake fallback (Stage 3-F)
# VISIBLE and COUNTABLE in `info`, per the research-fairness contract
# documented in docs/phase3/OVERNIGHT_PROGRESS.md's Stage 3-H entry:
# the fallback mechanism itself is kept (a legitimate execution-layer
# safety net, never a silent BehaviorAction override), but a high
# intervention rate must never be hidden behind a `success=True`
# aggregate outcome.
# ======================================================================


def test_intervention_fields_present_and_zero_at_episode_start_frenet_mpc(env):
    _, info = env.reset(SINGLE_MANEUVER)

    assert info["downstream_failure_count"] == 0
    assert info["planner_infeasible_count"] == 0
    assert info["collision_blocked_count"] == 0
    assert info["controller_failure_count"] == 0
    assert info["invalid_reference_count"] == 0
    assert info["intervention_rate"] == pytest.approx(0.0)


def test_intervention_counters_accumulate_across_steps_frenet_mpc(env):
    env.reset(SINGLE_MANEUVER)

    prev_failure_count = 0
    for _ in range(10):
        _, _, terminated, truncated, info = env.step(BehaviorAction.KEEP)
        # Counters are monotonically non-decreasing (cumulative tally).
        assert info["downstream_failure_count"] >= prev_failure_count
        prev_failure_count = info["downstream_failure_count"]
        # Exactly one of the three specific failure-type counters
        # accounts for downstream_failure_count minus any
        # invalid_reference occurrences (each step increments at most
        # one specific-status counter, and downstream_failure_count
        # whenever status != OK).
        assert info["downstream_failure_count"] == (
            info["planner_infeasible_count"]
            + info["collision_blocked_count"]
            + info["controller_failure_count"]
            + info["invalid_reference_count"]
        )
        assert 0.0 <= info["intervention_rate"] <= 1.0
        if terminated or truncated:
            break


def test_intervention_rate_matches_failure_count_over_steps_elapsed_frenet_mpc(env):
    env.reset(SINGLE_MANEUVER)
    ...
    expected_rate = info["downstream_failure_count"] / info["steps_elapsed"]
    assert info["intervention_rate"] == pytest.approx(expected_rate)


def test_intervention_counters_identical_regardless_of_policy_origin_frenet_mpc():
    """The counting logic has no notion of 'which policy chose this
    action' -- running the SAME deterministic action sequence through
    two independently-constructed MergeEnvironment(frenet_mpc)
    instances must produce byte-identical intervention counters,
    confirming the diagnostic is a pure function of downstream_status,
    never of a hidden policy-identity parameter."""

    action_sequence = [BehaviorAction.MERGE] * 10
    ...
    def run():
        ...
        info = None
        for action in action_sequence:
            _, _, terminated, truncated, info = env_instance.step(action)
            if terminated or truncated:
                break
        return info
    ...
    assert info_a["downstream_failure_count"] == info_b["downstream_failure_count"]
    assert info_a["planner_infeasible_count"] == info_b["planner_infeasible_count"]
    assert info_a["collision_blocked_count"] == info_b["collision_blocked_count"]
    assert info_a["controller_failure_count"] == info_b["controller_failure_count"]
    assert info_a["invalid_reference_count"] == info_b["invalid_reference_count"]
    assert info_a["intervention_rate"] == pytest.approx(info_b["intervention_rate"])


def test_intervention_counters_reset_on_new_episode_frenet_mpc(env):
    env.reset(SINGLE_MANEUVER)
    for _ in range(5):
    ...
    # A fresh reset (even on the same maneuver) must clear the
    # cumulative counters back to 0 -- they are episode-scoped, not
...
def test_intervention_fields_present_and_zero_in_legacy_mode():
    """Legacy mode has no planner/controller feasibility-collision
    check to intervene on -- these fields must still be PRESENT (so
    callers can read them uniformly across both modes) but always 0 /
    0.0, never populated with frenet_mpc-mode semantics."""
    ...
    for _ in range(5):
        ...
        assert step_info["downstream_failure_count"] == 0
        assert step_info["planner_infeasible_count"] == 0
        assert step_info["collision_blocked_count"] == 0
